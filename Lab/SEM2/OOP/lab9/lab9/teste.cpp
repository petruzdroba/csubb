...
static void testDeleteAllContract()
{
	...
	assert(service.getContractLungime() == 5);
...
static void testRandomPopulate()
{
	...
	service.randomPopulateContract(10);
	assert(service.getContractLungime() == 10);
}
...
{
	RepositoryDictionar repoZero{ 0 };

	assert(repoZero.getLungime() == 0);

	Disciplina disciplina{ "1", 1, "1", "1" };
	repoZero.adauga(disciplina);
	assert(repoZero.getLungime() == 1);

	repoZero.modifica(1, Disciplina("2", 2, "2", "2"));
	assert(repoZero.getAll()[0].getDenumire() == "2");
	...
	for (int index = 2; index <= 100; ++index) {
		repoZero.adauga(Disciplina{ to_string(index) ,index,to_string(index) ,to_string(index) });
		assert(repoZero.getLungime() == index);
	}

	int index2 = 1;
	for (Disciplina d : repoZero.getAll())
	{
		assert(d.getDenumire() == to_string(index2));
		assert(d.getOre() == index2);
		assert(d.getTip() == to_string(index2));
	...
	RepositoryDictionar repoOne{ 1 };

	try {
		repoOne.adauga(Disciplina("1", 1, "1", "1"));
	}
	catch (const MyException& e) {
		assert(string(e.getMessage()) == "adauga: unlucky repo");
	}
	...
	try {
		...
		assert(string(e.getMessage()) == "sterge: unlucky repo");
	}


	try {
		repoOne.modifica(1, Disciplina("2", 2, "2", "2"));
	}
	catch (const MyException& e) {
		assert(string(e.getMessage()) == "modifica: unlucky repo");
	}
}

static void testWriteFileDict()
{
	clearFile("test.txt");
	RepositoryDictionar repo{ 0, "test.txt" };
	Contract contract;
	Service service(repo, contract);
	assert(service.getLungimeService() == 0);
	...
	for (int index = 1; index <= 3; ++index)
	{
		...
		assert(firstField == repo.getAll()[index - 1].getDenumire());
	}

	file.close();
}

static void testOverrideFileDict()
{
	clearFile("test.txt");
	ofstream outFile("test.txt");

	outFile << "Matematica,5,Optional,Popescu\n";
	...
	Contract contract;
	Service service{ repo, contract };
	assert(service.getAllService()[0].getDenumire() == "Matematica");
	service.stergeService(0);
